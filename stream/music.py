...
@Client.on_message(command(["song", f"song@{CatX_botz.BOT_USERNAME}"]) & ~filters.channel)
def song(_, message):
    ...
    try:
        ...
        print(str(e))
        return
    m.edit("📥 downloading Requested Song...")
    try:
        with youtube_dl.YoutubeDL(ydl_ops) as ydl:
            info_dict = ydl.extract_info(link, download=False)
            audio_file = ydl.prepare_filename(info_dict)
            ydl.process_info(info_dict)
        rep = f"**🎧 By @{CatX_botz.BOT_USERNAME}**"
...
@Client.on_message(command(["vsong", f"vsong@{CatX_botz.BOT_USERNAME}"]) & filters.group & ~filters.edited)
