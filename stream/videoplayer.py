...
app = Client(CatX_botz.SESSION_NAME, CatX_botz.API_ID, CatX_botz.API_HASH)
...
@Client.on_message(command(["vplay", f"vplay@{CatX_botz.BOT_USERNAME}"]) & filters.group & ~filters.edited)
            ...
            [
                [
                    InlineKeyboardButton(
                        text="Support",
                        url=f"https://t.me/{CatX_botz.GROUP_NAME}"),
                    InlineKeyboardButton(
                        text="Update's",
                        url=f"https://t.me/{CatX_botz.CHANNEL_NAME}")
                ]
            ]
        )
    ...
    if not replied:
        if len(m.command) < 2:
            ...
            try:
                ...
                await m.reply_photo(
                    photo="https://te.legra.ph/file/145582d242bd9735252b5.png",
...
@Client.on_message(command(["vstop", f"vstop@{CatX_botz.BOT_USERNAME}"]) & filters.group & ~filters.edited)
@authorized_users_only
    ...
    try:
        ...
        if process:
            try:
                ...
                pass
        await call_py.leave_group_call(chat_id)
        await m.reply("✅ **Successfully lefted from videochat !**")
    except Exception as e:
...
@Client.on_message(command(["cplay", f"cplay@{CatX_botz.BOT_USERNAME}"]) & filters.group & ~filters.edited)
@authorized_users_only
...
@Client.on_message(command(["cstop", f"cstop@{CatX_botz.BOT_USERNAME}"]) & filters.group & ~filters.edited)
@authorized_users_only
